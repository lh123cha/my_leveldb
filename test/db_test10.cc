...
TEST(ColumnFamily, read){
  ...
  ColumnFamilyHandler cf1("test1");
  ColumnFamilyHandler cf2("test2");

  db->Put(WriteOptions(),"123",cf1, "123");
  db->Put(WriteOptions(),"321",cf2, "321");
...
TEST(ColumnFamily, iterator){
  ...
  for(int i=0;i<10;i++){
    db->Put(WriteOptions(), std::to_string(i), cf1,std::to_string(i));
  }
  for(int i=10;i<20;i++){
    db->Put(WriteOptions(), std::to_string(i),cf2, std::to_string(i));
  }
...
TEST(Index, PutWithIndex){
  ...
  ColumnFamilyHandler cf_record("Record");
  ColumnFamilyHandler cf_index("Index");
...
TEST(Index, Iterator){
  ...
  for(int i=0;i<10;i++){
    ...
    ASSERT_EQ(idx_iter->key().ToString(), std::to_string(91+i));//Get the value of the <Index_xxxxxdata_xxxxxkey>!!!
    ASSERT_EQ(idx_iter->value().ToString(), "");//judge whether the value is empty.
    idx_iter->Next();
  }
  delete idx_iter;
  delete db;
...
TEST(Index, scan){
  ...
  cnt = 0;
  iter = db->NewColumnFamilyIterator(ReadOptions(), *(new ColumnFamilyHandler("Record")));
  iter->SeekToFirst();
